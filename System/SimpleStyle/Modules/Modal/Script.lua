...
function loadAssets()
	...
	font_modal_header = loadFontRenderer(56, "regular")
    font_modal_body = loadFontRenderer(56, "regular")
	font_modal_plate = loadFontRenderer(10, "regular")
...
function draw()
	...
	if modal_current_type == 0 then
		...
		tx_header:t2D_DisplayImage_AnchorCenter(640,120)
		tx_body = getTextTex(ttk_modal_body, false, false)
		tx_body:t2D_DisplayImage_AnchorCenter(640,490)
	else
		-- Others
		...
		if modal_current_type == 1 then
			...
			if modal_current_visual ~= nil then
				modal_current_visual:t2D_DisplayImage(0,173)
			end
			tx_body = getTextTex(ttk_modal_body, false, false)
			tx_body:t2D_DisplayImage_AnchorCenter(640,260)
		elseif modal_current_type == 2 then
			-- Puchichara
			if modal_current_info.tx ~= nil then
				modal_current_info.tx:t2D_DisplayImage_AnchorCenter(640,327)
			end
			tx_body = getTextTex(ttk_modal_body, false, false)
			tx_body:t2D_DisplayImage_AnchorCenter(640,527)
		elseif modal_current_type == 3 then
			-- Nameplate Title
			tx_title = getTextTex(ttk_modal_body, false, false)
			modal_current_visual:DrawTitlePlate(640, 327, 255, modal_current_info.Value.nameplateInfo.iType, tx_title, modal_current_rarity, modal_current_info.Key)
		elseif modal_current_type == 4 then
			-- Song
			if modal_preimage_ref ~= nil then
				modal_preimage_ref:t2D_DisplayImage_AnchorCenter(640,327)
			end
			tx_body = getTextTex(ttk_modal_body, false, false)
			tx_body:t2D_DisplayImage_AnchorCenter(640,527)
		else
			...
			tx_body = getTextTex(ttk_modal_body, false, false)
			tx_body:t2D_DisplayImage_AnchorCenter(640,327)
		end 
	end
